...
const CheckOutPage = () => {
  ...
  const handlePayment = async (totalAmount) => {
    try {
      ...
      const { data: { order } } = await axios.post("http://localhost:4000/api/payment/paynow", { totalAmount });
      ...
      const options = {
        ...
        handler: function (response) {
          ...
          try {
            ...
            axios.post('http://localhost:4000/api/place-order-cod', data, config)
              .then(async (response) => {
      
                await axios.delete('http://localhost:4000/api/delete-cart', config)
                  .then(() => {
