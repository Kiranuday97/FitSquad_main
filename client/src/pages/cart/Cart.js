...
const Cart = () => {
  ...
  const fetchCartItems = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:4000/api/cart', token);
  ...
  const incrementCount = async (productId) => {
    try {
      ...
      await axios.put(`http://localhost:4000/api/cart-update-quantity/${productId}/increment`, null, config);
  ...
  const decrementCount = async (productId) => {
    try {
      ...
      await axios.put(`http://localhost:4000/api/cart-update-quantity/${productId}/decrement`, null, config);
            ...
            <tbody>
              {cartItems &&
                ...
                cartItems.cart.products.map((item, index) => (
                  <tr key={index}>
                    <td>
                      <img src={`http://localhost:4000/${item.productId.imageUrl}`} alt={item.productId.name} className="item-image" />
                    </td>
